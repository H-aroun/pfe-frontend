...
export default function AnalyticsPage() {
  ...
  const { data, isLoading } = useQuery<AnalyticsDashboard>({
    queryKey: ['analytics-full', isAdmin ? 'admin' : 'me'],
    queryFn: () =>
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">
            {isAdmin ? 'Platform-wide performance overview' : 'Your scenario performance overview'}
          </p>
        </div>
                ...
                {trendData.length ? (
                  <ResponsiveContainer width="100%" height={220}>
                    <AreaChart data={trendData}>
                      ...
                      <Area type="monotone" dataKey="count" stroke="var(--lux-primary)" fill="url(#compGrad)" strokeWidth={2} dot={false} name="Activity" />
                  ...
                  <div className="flex h-[220px] items-center justify-center text-xs text-slate-600">
                    No trend data yet
                  </div>
                )}
              </CardBody>
                ...
                {hasScores ? (
                  <ResponsiveContainer width="100%" height={220}>
                    <BarChart data={scoreDistribution}>
                      ...
                      <Tooltip contentStyle={tooltipStyle} />
                      <Bar dataKey="count" radius={[4, 4, 0, 0]} name="Scenarios">
